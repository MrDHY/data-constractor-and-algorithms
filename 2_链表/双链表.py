...
def test_double_linked_list():
    ...
    assert [node.value for node in dll.iter_item()] == [0, 1, 2]
    assert [node.value for node in dll.reverse()] == [2, 1, 0]

    headnode = dll.head()
    assert headnode.value == 0
    dll.remove(headnode)
    assert len(dll) == 2
    assert [node.value for node in dll.iter_item()] == [1, 2]
